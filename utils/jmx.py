...
def jmx_command(args, agent_config, redirect_std_streams=False):
    ...
    if len(args) < 1 or args[0] not in JMX_LIST_COMMANDS.keys():
        ...
        print "See http://support.oneapm.com/ for more information"
        print "#" * 80
        print "\n"
        print "You have to specify one of the following commands:"
        for command, desc in JMX_LIST_COMMANDS.iteritems():
            print "      - %s [OPTIONAL: LIST OF CHECKS]: %s" % (command, desc)
        print "Example: sudo /etc/init.d/oneapm-ci-agent jmx list_matching_attributes tomcat jmx solr"
        print "\n"
        ...
        if should_run:
            ...
            print "If you think it's not normal please get in touch with OneAPM Support"
...
class JMXFiles(object):
    ...
    def _get_dir(cls):
        if Platform.is_win32():
            path = os.path.join(_windows_commondata_path(), 'OneAPM')
