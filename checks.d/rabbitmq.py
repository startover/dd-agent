...
class RabbitMQ(AgentCheck):
    ...
    def get_stats(self, instance, base_url, object_type, max_detailed, filters, auth=None):
        ...
        if len(data) > max_detailed:
            ...
            self.warning(
                "Too many queues to fetch. You must choose the %s you are interested in by editing the rabbitmq.yaml configuration file or get in touch with OneAPM Support" % object_type)
    ...
    def alert(self, base_url, max_detailed, size, object_type):
        ...
        Please get in touch with OneAPM support to increase the limit.""" % (size, object_type, max_detailed)
