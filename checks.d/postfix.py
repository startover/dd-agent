...
class PostfixCheck(AgentCheck):
    ...
    WARNING: the user that oneapm-ci-agent runs as must have sudo access for the 'find' command
             sudo access is not required when running oneapm-ci-agent as root (not recommended)

    example /etc/sudoers entry:
             oneapm-ci-agent ALL=(ALL) NOPASSWD:/usr/bin/find
    ...
    def _get_queue_count(self, directory, queues, tags):
        for queue in queues:
            ...
            count = 0
            if os.geteuid() == 0:
                # oneapm-ci-agent is running as root (not recommended)
                count = sum(len(files) for root, dirs, files in os.walk(queue_path))
            else:
                # can oneapm-ci-agent user run sudo?
                test_sudo = os.system('setsid sudo -l < /dev/null')
                if test_sudo == 0:
                    count = os.popen('sudo find %s -type f | wc -l' % queue_path)
                    count = count.readlines()[0].strip()
                else:
                    raise Exception('The oneapm-ci-agent user does not have sudo access')
