...
class AgentMetrics(AgentCheck):
    ...
    def _register_psutil_metrics(self, stats, names_to_metric_types):
        ...
        :param stats: a dictionary that looks like:
        {
         ...
         This creates a metric like `oneapm.agent.collector.{key_1}.{key_2}` where key_1 is a top-level
         key in `stats`, and key_2 is a nested key.
         E.g. oneapm.agent.collector.memory_info.rss
        """

        base_metric = 'oneapm.agent.collector.{0}.{1}'
        ...
        for k, v in stats.iteritems():
            ...
            if isinstance(v, dict):
                ...
                full_metric_name = 'oneapm.agent.collector.{0}'.format(k)
    ...
    def check(self, instance):
        ...
        if threading.activeCount() > MAX_THREADS_COUNT:
            self.gauge('oneapm.agent.collector.threads.count', threading.activeCount())
        ...
        if collection_time is not None and \
            ...
            self.gauge('oneapm.agent.collector.collection.time', collection_time)
        ...
        if emit_time is not None and \
            ...
            self.gauge('oneapm.agent.emitter.emit.time', emit_time)
        ...
        if cpu_time is not None:
            try:
                ...
                if cpu_used_pct > MAX_CPU_PCT:
                    self.gauge('oneapm.agent.collector.cpu.used', cpu_used_pct)
