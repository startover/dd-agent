...
class DdForwarder(object):
    ...
    def check(self, agentConfig, move_end=True):
        ...
        if self.log_path and os.path.isfile(self.log_path):
            ...
            self.logger.debug("Can't tail agent forwarder log file: %s" % self.log_path)
            return {}
...
def testddForwarder():
    ...
    logger = logging.getLogger("ciagent.checks.agent")
    logger.setLevel(logging.DEBUG)
    logger.addHandler(logging.StreamHandler())

    config = {'license_key': 'my_licensekey', 'ddforwarder_log': sys.argv[1]}
