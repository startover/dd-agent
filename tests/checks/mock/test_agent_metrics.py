...
class AgentMetricsTestCase(AgentCheckTest):
    ...
    def test_send_single_metric(self):
        ...
        check._send_single_metric('oneapm.agent.collector.memory_info.vms', 16814081, 'gauge')
        check.gauge.assert_called_with('oneapm.agent.collector.memory_info.vms', 16814081)

        check._send_single_metric('oneapm.agent.collector.memory_info.vms', 16814081, 'rate')
        check.rate.assert_called_with('oneapm.agent.collector.memory_info.vms', 16814081)

        self.assertRaises(Exception, check._send_single_metric,
                          *('oneapm.agent.collector.memory_info.vms', 16814081, 'bogus'))
    ...
    def test_register_psutil_metrics(self):
        ...
        self.assertMetric('oneapm.agent.collector.memory_info.rss', value=16814080)
        self.assertMetric('oneapm.agent.collector.memory_info.vms', value=74522624)
