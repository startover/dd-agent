...
if [ ! $license_key ]; then
    printf "\033[31mAPI key not available in CI_LICENSE_KEY environment variable.\033[0m\n"
    exit 1;
fi
...
if [ -e /etc/oneapm-ci-agent/oneapm-ci-agent.conf ]; then
    ...
    printf "\033[34m\n* Adding your API key to the Agent configuration: /etc/oneapm-ci-agent/oneapm-ci-agent.conf\n\033[0m\n"
...
$cl_cmd http://127.0.0.1:17123/status?threshold=0 > /dev/null 2>&1
success=$?
